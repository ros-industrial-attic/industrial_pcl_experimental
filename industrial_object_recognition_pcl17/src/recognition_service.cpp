...
std::ofstream STATFILE_("/home/jnicho/Desktop/pose_measurements.csv",
		ios::out | ios::app);
//STATFILE_.open( filename.c_str(), ios::out | ios::app );
...
public:
	...
	ros::Publisher object_pub_, model_pub_, object_kp_pub_, model_kp_pub_,
			pose_pub_, grip_pose_pub_;
	...
	RecognitionNode(ros::NodeHandle nh) :
		nh_(nh), priv_nh_("~"), model_ss_(0.01), scene_ss_(0.03),
				...
				use_uniform_sampling_(true), min_scale_(0.0005),
				nr_octaves_(4), nr_scales_per_octave_(5), min_contrast_(1),
				use_VFH_(true), use_SACICP_(true), min_sample_distance_(0.0),
				nr_iterations_(3), transformation_epsilon_(0.0),
				max_iterations_(1000), max_corr_distance_(0.05),
				x_filter_min_(-1.0), x_filter_max_(1.0), y_filter_min_(-1.0),
				y_filter_max_(1.0), z_filter_min_(-1.0), z_filter_max_(1.0),
				tesselation_level_(1), tess_view_angle_(60.0), sor_mean_(10),
				sor_thresh_(1.0), num_images_(1), test_mode_(false) {
		//tabletop_seg_client_ = nh_.serviceClient<tabletop_object_detector::TabletopSegmentation>(TABLETOP_SEGMENTATION);

		object_pub_ = nh.advertise<sensor_msgs::PointCloud2> ("object_cluster",
				1);
		model_pub_
				= nh.advertise<sensor_msgs::PointCloud2> ("model_cluster", 1);
		object_kp_pub_
				= nh.advertise<sensor_msgs::PointCloud2> ("object_kp", 1);
		...
		grip_pose_pub_ = nh.advertise<geometry_msgs::PoseStamped> (
				"grasp_pose", 1);
	...
	bool initRecognizers() {
		if (model_list_.size() > 0) {
			...
			for (std::size_t i = 0; i < model_list_.size(); i++) {
				...
				pcl17::PointCloud<PointType>::Ptr model_ptr(
						new pcl17::PointCloud<PointType>(
								rec.model_data_.points_));
				if (use_hough_) {
	...
	bool initModelData(std::string & name, CloudData & model_data) {
		...
		all_descriptors1_ = pcl17::PointCloud<Descriptor1Type>::Ptr(
				new pcl17::PointCloud<Descriptor1Type>);
		all_descriptors2_ = pcl17::PointCloud<Descriptor2Type>::Ptr(
				new pcl17::PointCloud<Descriptor2Type>);
	...
	bool initCloudData(float radius_search, std::string & name,
		...
		pcl17::PointCloud<PointType>::Ptr model_ptr(
				new pcl17::PointCloud<PointType>(data.points_));
		;
		...
		pcl17::PointCloud<NormalType>::Ptr norms_ptr(
				new pcl17::PointCloud<NormalType>(data.norms_));
		...
		if (use_uniform_sampling_) {
			...
			pcl17::copyPointCloud(*model_ptr, sampled_indices.points,
					data.keypoints_);
			...
			pcl17::search::KdTree<pcl17::PointNormal>::Ptr SIFT_tree(
					new pcl17::search::KdTree<pcl17::PointNormal>());
			;//new API
			pcl17::PointCloud<pcl17::PointWithScale>::Ptr sifts(
					new pcl17::PointCloud<pcl17::PointWithScale>);
			pcl17::PointCloud<pcl17::PointNormal>::Ptr cloud_with_normals(
					new pcl17::PointCloud<pcl17::PointNormal>);
			pcl17::concatenateFields(*model_ptr, *norms_ptr,
					*cloud_with_normals);
		...
		if (use_VFH_) {
			...
			pcl17::search::KdTree<PointType>::Ptr VFH_tree(
					new pcl17::search::KdTree<PointType>());
			...
			pcl17::PointCloud<PointType>::Ptr keypoints_ptr(
					new pcl17::PointCloud<PointType>(data.keypoints_));

			pcl17::SHOTEstimationOMP<PointType, NormalType, Descriptor1Type>
					descr_est;
			priv_nh_.getParamCached("descr_rad", descr_rad_);
	...
	bool recognizeModel(pcl17::PointCloud<PointType> & scene, Recognizer & rec) {
		...
		if (use_VFH_) {
			pcl17::KdTreeFLANN<Descriptor2Type>::Ptr VFH_search(
					new pcl17::KdTreeFLANN<Descriptor2Type>);
			...
			const Descriptor2Type query_descriptor =
					rec.rec_data_.scene_.descriptors2_.points[0];// .global_descriptor->points[0];
			VFH_search->nearestKSearch(query_descriptor, 1, nn_index,
					nn_sqr_dists);
			...
			pcl17::PointCloud<PointType> mod_cloud =
					models_[best_match_].points_;
			...
			ros::Duration init_model_total = init_model_finish
					- init_both_start;
			initCloudData(scene_ss_, name, source_name, scene,
					rec.rec_data_.scene_);
		...
		if (use_SACICP_) {
			...
			pcl17::PointCloud<PointType>::Ptr model_pt_ptr(
					new pcl17::PointCloud<PointType>(rec.model_data_.points_));
			pcl17::PointCloud<PointType>::Ptr scene_pt_ptr(
					new pcl17::PointCloud<PointType>(
							rec.rec_data_.scene_.points_));
			pcl17::PointCloud<PointType>::Ptr
					model_kp_ptr(
							new pcl17::PointCloud<PointType>(
									rec.model_data_.keypoints_));
			pcl17::PointCloud<PointType>::Ptr scene_kp_ptr(
					new pcl17::PointCloud<PointType>(
							rec.rec_data_.scene_.keypoints_));
			pcl17::PointCloud<Descriptor1Type>::Ptr model_desc_ptr(
					new pcl17::PointCloud<Descriptor1Type>(
							rec.model_data_.descriptors1_));
			pcl17::PointCloud<Descriptor1Type>::Ptr scene_desc_ptr(
					new pcl17::PointCloud<Descriptor1Type>(
							rec.rec_data_.scene_.descriptors1_));
			pcl17::SampleConsensusInitialAlignment<PointType, PointType,
					Descriptor1Type> sac_ia;
			...
			pcl17::PointCloud<PointType>::Ptr source_points_transformed(
					new pcl17::PointCloud<PointType>);
			...
			pcl17::transformPointCloud(*model_pt_ptr,
					*source_points_transformed, sac_ia_tform);
			...
			pcl17::transformPointCloud(rec.model_data_.points_,
					transformed_model, icp_tform);
			...
			pcl17::transformPointCloud(rec.model_data_.keypoints_,
					transformed_model_kp, icp_tform);
			...
			for (size_t i = 0; i < rec.rec_data_.scene_.descriptors1_.size(); ++i) {
				...
				int found_neighs = shot_match_search.nearestKSearch(
						rec.rec_data_.scene_.descriptors1_.at(i), 1,
				...
				{
					pcl17::Correspondence corr(neigh_indices[0],
			...
			ros::Duration correspondence_total = correspondence_finish
					- registration_start;
			...
			pcl17::PointCloud<PointType>::Ptr scene_kp_ptr(
					new pcl17::PointCloud<PointType>(
							rec.rec_data_.scene_.keypoints_));
			pcl17::CorrespondencesPtr
					model_scene_corrs(
							new pcl17::Correspondences(
									rec.rec_data_.model_scene_corrs));

			pcl17::PointCloud<NormalType>::Ptr model_norm_ptr(
					new pcl17::PointCloud<NormalType>(rec.model_data_.norms_));
			//pcl17::PointCloud<NormalType>::Ptr model_norm_ptr(new pcl17::PointCloud<NormalType>(all_normals_));
			pcl17::PointCloud<PointType>::Ptr model_pt_ptr(
					new pcl17::PointCloud<PointType>(rec.model_data_.points_));
			pcl17::PointCloud<NormalType>::Ptr scene_norm_ptr(
					new pcl17::PointCloud<NormalType>(
							rec.rec_data_.scene_.norms_));
			pcl17::PointCloud<PointType>::Ptr scene_pt_ptr(
					new pcl17::PointCloud<PointType>(
							rec.rec_data_.scene_.points_));
			...
			if (use_hough_) {
				...
				priv_nh_.getParamCached("rf_radius", rf_rad_);
				pcl17::PointCloud<RFType>::Ptr model_rf(
						new pcl17::PointCloud<RFType>());
				pcl17::PointCloud<RFType>::Ptr scene_rf(
						new pcl17::PointCloud<RFType>());
				pcl17::BOARDLocalReferenceFrameEstimation<PointType,
						NormalType, RFType> rf_est;
				...
				rec.rec_data_.h_clusterer_.setModelSceneCorrespondences(
						model_scene_corrs);

				rec.rec_data_.h_clusterer_.recognize(rototranslations,
						clustered_corrs);
				...
				if (cg_thresh_ == -1) {
					rec.rec_data_.percent_match_ = float(
							clustered_corrs[0].size()) / float(
							rec.rec_data_.model_scene_corrs.size());
					...
					pcl17::PointCloud<PointType>::Ptr scene_pt_ptr(
							new pcl17::PointCloud<PointType>(
									rec.rec_data_.scene_.points_));
					pcl17::PointCloud<PointType>::Ptr model_kp_ptr(
							new pcl17::PointCloud<PointType>(
									rec.model_data_.keypoints_));
					pcl17::PointCloud<PointType>::Ptr scene_kp_ptr(
							new pcl17::PointCloud<PointType>(
					...
					pcl17::PointCloud<Descriptor1Type>::Ptr scene_desc_ptr(
							new pcl17::PointCloud<Descriptor1Type>(
									rec.rec_data_.scene_.descriptors1_));
					pcl17::SampleConsensusInitialAlignment<PointType,
							PointType, Descriptor1Type> sac_ia;
					pcl17::PointCloud<PointType> registration_output;
					...
					pcl17::transformPointCloud(*model_pt_ptr,
							*source_points_transformed, hough_tform);
				...
				ros::Duration registration_total = registration_finish
						- registration_start;
				...
				rec.rec_data_.gc_clusterer_.setModelSceneCorrespondences(
						model_scene_corrs);
				rec.rec_data_.gc_clusterer_.setGCSize(cg_size_);
				rec.rec_data_.gc_clusterer_.setGCThreshold(cg_thresh_);

				rec.rec_data_.gc_clusterer_.recognize(rototranslations,
						clustered_corrs);
			}
			...
			for (size_t j = 0; j < rototranslations.size(); ++j) {
				...
				/*        //GET TF OF CURRENT FRAME TO WORLD FRAME
				 ...
				 std::string clusterFrameId = rec.rec_data_.scene_.points_.header.frame_id;
				 ROS_INFO_STREAM("Scene passed to recognizer with frame id: "<<rec.rec_data_.scene_.points_.header.frame_id);
				 _WorldFrameId = world_frame_;
				 try
				 {
				 _tfListener.waitForTransform(_WorldFrameId, clusterFrameId, ros::Time::now(), ros::Duration(1.0));
				 _tfListener.lookupTransform(_WorldFrameId ,clusterFrameId, ros::Time(0),clusterTf);
				 }
				 ...
				 tf::TransformTFToEigen(clusterTf,tfEigen);//clusterTf from ClusterFrame to WorldFrame
				 pcl17::transformPointCloud(transformed_model1,transformed_model,Eigen::Affine3f(tfEigen));
				 */
				sensor_msgs::PointCloud2 model_msg;
				pcl17::toROSMsg(transformed_model, model_msg);
				model_msg.header.frame_id = world_frame_;
				...
				tf_mod_matrix.setValue(model_rot(0, 0), model_rot(0, 1),
						model_rot(0, 2), model_rot(1, 0), model_rot(1, 1),
						model_rot(1, 2), model_rot(2, 0), model_rot(2, 1),
						model_rot(2, 2));
				...
				Eigen::Matrix3f rotation = rototranslations[j].block<3, 3> (0,
						0);
				Eigen::Vector3f translation = rototranslations[j].block<3, 1> (
						0, 3);

				tf::Matrix3x3 tf_matrix;
				tf_matrix.setValue(rotation(0, 0), rotation(0, 1),
						rotation(0, 2), rotation(1, 0), rotation(1, 1),
						rotation(1, 2), rotation(2, 0), rotation(2, 1),
						rotation(2, 2));
				tf::Vector3 tf_translation;
				tf_translation.setValue(translation(0), translation(1),
						translation(2));
				...
				ROS_INFO_STREAM("Angle between z component of approach vector and z normal to table: " << angle_off*180/3.14159);
				...
				gripper_pose_.setOrigin(
						tf::Vector3(part_pose_xy.x(), part_pose_xy.y(), 0.080));//part_pose_xy.z()
				...
				//tf::Vector3 dot_x=
				tf::Vector3 x_proj = x_v_p - tf::tfDot(x_v_p, grip_z_v)
						* grip_z_v;
				...
				grip_r.setValue(x_proj.getX(), grip_y_v.getX(), 0,
						x_proj.getY(), grip_y_v.getY(), 0, x_proj.getZ(),
						grip_y_v.getZ(), -1.0f);
	...
	void spin() {
		...
		for (int k = 0; k < 50; k++) {
			if (ros::ok()) {
				...
				pcl17::PointCloud<PointType>::Ptr cloud_ptr(
						new pcl17::PointCloud<PointType>(cloud));
				...
				pcl17::PointCloud<PointType>::Ptr f_cloud_ptr(
						new pcl17::PointCloud<PointType>(xf_cloud));
				...
				sensor_msgs::PointCloud2::Ptr pc2_cloud(
				...
				if (yf_cloud.size() != 0) {
					...
					ros::Duration rec_total = rec_total_finish
							- rec_total_start;
	...
	bool rec_CB(mantis_perception::mantis_recognition::Request &main_request,
		...
		if (test_mode_)
		{
			...
			do {
				...
				try {
					tf_listener.waitForTransform(processing_frame,
							clusterFrameId, ros::Time::now(),
							ros::Duration(1.0));
					tf_listener.lookupTransform(processing_frame,
							clusterFrameId, ros::Time(0), sceneTf);
				...
				pcl17::transformPointCloud(cloud, transformed_scene,
						Eigen::Affine3f(tfEigen_scene));

				//Passthrough filters
				pcl17::PointCloud<PointType>::Ptr cloud_ptr(
						new pcl17::PointCloud<PointType>(transformed_scene));
				pcl17::PassThrough<pcl17::PointXYZ> pass_x;
				pass_x.setInputCloud(cloud_ptr);
				pass_x.setFilterFieldName("x");
				pass_x.setFilterLimits(x_filter_min_, x_filter_max_);
				...
				pcl17::PointCloud<PointType>::Ptr fil_cloud_ptr(
						new pcl17::PointCloud<PointType>(yf_cloud));
				...
				pcl17::PointCloud<pcl17::PointXYZI>::Ptr pre_cloud(
						new pcl17::PointCloud<pcl17::PointXYZI>);
				...
				if (filtered_cloud.size() != 0) {
					...
					if (acceptable_pose) {
						...
						tf::Quaternion pose_rot_forfile =
								gripper_pose_.getRotation();
	...
	void init_srv() {
		...
		recognition_server_ = nh_.advertiseService(
				"/mantis_object_recognition", &RecognitionNode::rec_CB, this);
		//sample_number_=0;
		ros::spin();
	}
