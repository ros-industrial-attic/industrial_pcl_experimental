...
*/
  ...
  bool initCloudData(float radius_search, std::string & name,
                     std::string & source_name, pcl::PointCloud<PointType> & cloud,
                     CloudData & data)
  {
...
;
    ...
    else
    {
      ...
      pcl::search::KdTree<pcl::PointNormal>::Ptr SIFT_tree(new pcl::search::KdTree<pcl::PointNormal> ());;//new API
  ...
  void recognize(pcl::PointCloud<PointType> & scene)
  {
    use_VFH_=true;
  ...
  void recognizeModel(pcl::PointCloud<PointType> & scene, Recognizer & rec)
  {
    ROS_INFO_STREAM("#####################################################");
    ...
    if (use_SACICP_)
    {
      ...
      pcl::PointCloud<Descriptor1Type>::Ptr model_desc_ptr(new pcl::PointCloud<Descriptor1Type>(rec.model_data_.descriptors1_));
      pcl::PointCloud<Descriptor1Type>::Ptr scene_desc_ptr(new pcl::PointCloud<Descriptor1Type>(rec.rec_data_.scene_.descriptors1_));
